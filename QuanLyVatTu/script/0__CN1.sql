...
BEGIN
    ...
    DECLARE @LoginName VARCHAR(50) = N'cn1_'+ @Username;
    ...
    BEGIN TRY
        ...
        SET @SQL = N'USE CN1; CREATE USER [' + @LoginName + N'] FOR LOGIN [' + @LoginName + N'];';
        EXEC (@SQL);
    
        ---------------------------------------------------------
        SET @CurrentStep = N'A3 - ADD DATABASE ROLE';
        ---------------------------------------------------------
        SET @SQL =N'USE CN1; ALTER ROLE [' + @Role + N'] ADD MEMBER [' + @LoginName + N'];';
        BEGIN TRY
            EXEC(@SQL);
        END TRY
        BEGIN CATCH
